...
class Scheduler(object):
    ...
    async def _run_pending(self, nprod: int, ncon: int):
        runnable_jobs = (self._run_job(job) for job in self.jobs if job.should_run)


        await asyncio.gather(*runnable_jobs)

        # for job in sorted(runnable_jobs):
        #     self._run_job(job)

    def run_pending(self, nprod: int = 4, ncon: int = 4):
        """
        ...
        """
        asyncio.run(self._run_pending(nprod, ncon))
